...
class SchemaCollectorTest {
  ...
  void collectsSimpleObjectWithStringField() {
    ...
    assertFalse(field.needsYamlProperty());
  }
  ...
  void sanitizesHyphenatedFieldName() {
    ...
    assertTrue(field.needsYamlProperty());
  }
  ...
  @Test
  void collectsRefToStandaloneOneOfMixedAsObject() {
    GeneratorModel model =
        collect(
            """
                $defs:
  ...
  @Test
  void collectsRefToStandaloneAnyOfStringsAsObject() {
    GeneratorModel model =
        collect(
            """
                $defs:
  ...
  @Test
  void collectsRefToStandaloneOneOfAllRefsWithTypes() {
    GeneratorModel model =
        collect(
            """
                $defs:
  ...
  @Test
  void collectsRefToStandaloneAnyOfAllRefsWithTypes() {
    GeneratorModel model =
        collect(
            """
                $defs:
    ...
    JavaField field =
        model.classes().stream()
            ...
            .get(0);
    assertEquals("Object", field.javaType());
    assertEquals(List.of("Fast", "Slow"), field.oneOfTypes());
  }

  @Test
  void collectsNestedRefThroughStandaloneOneOf() {
    GeneratorModel model =
        collect(
            """
                $defs:
    ...
    JavaClass root =
        model.classes().stream()
            ...
            .orElseThrow();
    JavaField first =
        root.fields().stream().filter(f -> f.fieldName().equals("first")).findFirst().orElseThrow();
    JavaField second =
        root.fields().stream()
            .filter(f -> f.fieldName().equals("second"))
            .findFirst()
            .orElseThrow();
    assertEquals("Object", first.javaType());
    assertEquals("Object", second.javaType());
    assertEquals(first.oneOfTypes(), second.oneOfTypes());
  }

  @Test
  void standaloneOneOfDoesNotGeneratePhantomClass() {
    GeneratorModel model =
        collect(
            """
                $defs:
                ...
                properties:
                  target:
                    $ref: '#/$defs/endpoint'
                """);
    boolean hasEndpointClass =
        model.classes().stream().anyMatch(c -> c.className().equals("Endpoint"));
    assertFalse(hasEndpointClass, "standalone oneOf should not generate a class named Endpoint");
    JavaField field =
        model.classes().stream()
            .filter(c -> c.className().equals("Root"))
            .findFirst()
  ...
  @Test
  void collectsRefToStandaloneOneOfSingleObjectRefAsObject() {
    GeneratorModel model =
        collect(
            """
                $defs:
    ...
    JavaField field =
        model.classes().stream()
            ...
            .get(0);
    assertEquals("Object", field.javaType());
    assertEquals(List.of("Config"), field.oneOfTypes());
  }
}
