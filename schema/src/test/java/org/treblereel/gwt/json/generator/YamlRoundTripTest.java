...
import org.treblereel.gwt.json.generator.workflow.Root_YamlMapperImpl;
...
class YamlRoundTripTest {
  ...
  void roundTrip(String yamlFile) throws Exception {
    ...
    String yamlSource = loadYamlString(yamlFile);
    Root deserialized;
    try {
      deserialized = Root_YamlMapperImpl.INSTANCE.read(yamlSource);
    } catch (ClassCastException e) {
      assumeTrue(
          false,
          yamlFile
              + ": skipped — mapper-yaml does not support non-string YAML keys: "
              + e.getMessage());
      return;
    }
    assertNotNull(deserialized, "Deserialization returned null for " + yamlFile);

    String roundTrippedYaml = Root_YamlMapperImpl.INSTANCE.write(deserialized);
    JsonNode roundTrippedTree = YAML_MAPPER.readTree(roundTrippedYaml);
  ...
  private String loadYamlString(String yamlFile) throws IOException {
    try (InputStream is = getClass().getResourceAsStream("/workflows/" + yamlFile)) {
      assertNotNull(is, "YAML file not found: " + yamlFile);
